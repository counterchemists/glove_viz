...
public class InverseKinematics : MonoBehaviour {
    ...
    [Space(20)]
    public bool handMatchesPreviousJointRotation = true;
    [Space(20)]
    public bool handMatchesCopyPreviousRotation = false;
    [Space(20)]
	public bool debug;
	...
	void LateUpdate () {
		if(upperArm != null && forearm != null && hand != null && elbow != null && target != null){
			...
			if(handMatchesPreviousJointRotation)
            {
                hand.localRotation = Quaternion.Euler(forearm.localEulerAngles.x, forearm.localEulerAngles.y, forearm.localEulerAngles.z);
                hand.Rotate(hand_OffsetRotation);
            }

            if(handMatchesCopyPreviousRotation)
            {
                Vector3 rot = forearm.localPosition - upperArm.localPosition;
                Debug.Log(rot);
                // Gizmos.DrawLine(upperArm.position, forearm.position);
                //  Gizmos.DrawLine(forearm.position, hand.position);
                // hand.rotation = upperArm.rotation:// * Quaternion.EulerAngles(Vector3.;
                // hand.localRotation = Quaternion.Euler(rot);// Quaternion.Euler(forearm.localEulerAngles.x, forearm.localEulerAngles.y, forearm.localEulerAngles.z);
                //  Quaternion.Euler(forearm.localEulerAngles.x, forearm.localEulerAngles.y, -forearm.localEulerAngles.z);



                Quaternion q = vectorRotationQ(forearm.localPosition, hand.localPosition);

                Vector3 r = upperArm.position- target.position;
                Debug.DrawRay(hand.position, r + hand_OffsetRotation, Color.green);

                target.rotation = Quaternion.Euler(new Vector3(forearm.rotation.x, forearm.rotation.y, -forearm.rotation.z));
                //  hand.Rotate(hand_OffsetRotation);
                hand.rotation = target.rotation;
                hand.Rotate(hand_OffsetRotation);
            }
